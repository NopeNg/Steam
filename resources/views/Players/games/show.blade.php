...
<style>
    #steam-thumb-container::-webkit-scrollbar {
        display: none; /* Khóa scrollbar trên Chrome, Safari, Edge */
    }
    #steam-thumb-container {
        -ms-overflow-style: none;  /* Khóa scrollbar trên IE/Edge cũ */
        scrollbar-width: none;  /* Khóa scrollbar trên Firefox */
    }
</style>
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-[#171a21] p-4 rounded-sm border border-[#2a475e]/20 shadow-2xl">
        ...
        <div class="lg:col-span-2 space-y-3">
            ...
            <div class="h-64 md:h-96 rounded-sm overflow-hidden shadow-inner relative group select-none bg-black">
                <img id="main-game-image" src="{{ $game->cover_image }}" alt="{{ $game->name }}" class="w-full h-full object-cover transition-all duration-500 ease-in-out">
                
                <button onclick="prevImage()" class="absolute left-3 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-[#2a475e] text-white p-2 rounded-sm border border-[#2a475e]/40 transition-all opacity-0 group-hover:opacity-100 flex items-center justify-center w-10 h-12 z-10">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                    </svg>
                </button>

                <button onclick="nextImage()" class="absolute right-3 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-[#2a475e] text-white p-2 rounded-sm border border-[#2a475e]/40 transition-all opacity-0 group-hover:opacity-100 flex items-center justify-center w-10 h-12 z-10">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
            ...
            <div class="bg-black/30 p-1.5 rounded-sm select-none" 
                 id="game-gallery-wrapper" 
                ...
                <div class="mt-2 flex items-center justify-between bg-[#233c51]/10 h-5 rounded-xs border border-[#233c51]/20 px-2 text-[10px] text-[#67c1f5]">
                    <button onclick="scrollThumbsHandled('left')" class="hover:text-white transition p-1 select-none">◀</button>
                    
                    <div id="steam-slider-track" class="flex-1 h-2 bg-[#101822] mx-4 rounded-xs relative cursor-pointer">
                        <div id="steam-slider-handle" class="absolute left-0 top-0 bottom-0 w-16 bg-[#233c51] hover:bg-[#315371] active:bg-[#416e94] rounded-xs cursor-grab active:cursor-grabbing transition-shadow duration-150"></div>
                    </div>
                    
                    <button onclick="scrollThumbsHandled('right')" class="hover:text-white transition p-1 select-none">▶</button>
                </div>
            </div>
        ...
        <div class="flex flex-col justify-between space-y-4">
            <div class="space-y-3">
                ...
                <div class="text-xs space-y-1.5 pt-3 border-t border-[#2a475e]/20">
                    ...
                    <p class="text-[#556772]">Nhà phát hành: <span class="text-[#8f98a0]">{{ $game->publisher ?? 'Đang cập nhật' }}</span></p>
    ...
    <div class="space-y-3 pt-2">
        <h2 class="text-white text-sm uppercase tracking-widest font-bold border-b border-[#2a475e]/30 pb-2">Yêu cầu hệ thống</h2>
        <div class="bg-[#171a21]/60 border border-[#2a475e]/20 rounded-sm p-4 text-xs text-[#8f98a0] leading-relaxed tracking-wide font-mono">
