                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-4 flex-grow">
                        ...
                        <p class="text-[#c7d5e0] text-sm mb-4">
                            Rất tiếc, hệ thống gặp sự cố khi xử lý đơn hàng của bạn. Chúng tôi đang nỗ lực khắc phục sớm nhất có
                            thể.
                        </p>
                        <div class="bg-[#101822] p-4 rounded-sm mb-4 border border-[#2a475e]/20">
                            <p class="text-white text-xs font-bold mb-2">Mã đơn hàng: <span
                                    class="text-sky-400">#ORD-{{ $order->id }}</span></p>
                            <p class="text-white text-xs font-bold">Tổng tiền: <span
                                    class="text-emerald-400">{{ number_format($order->total_amount) }}đ</span></p>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="text-[#c7d5e0] text-xs space-y-1">
                                <p><i class="fas fa-envelope me-2 text-sky-400"></i><strong>Email:</strong> <a
                                        href="mailto:admin@gamekey.com"
                                        class="text-sky-400 hover:underline">admin@gamekey.com</a></p>
                                <p><i class="fas fa-phone me-2 text-sky-400"></i><strong>Điện thoại:</strong> <a
                                        href="tel:+84123456789" class="text-sky-400 hover:underline">+84 (123) 456-789</a></p>
                                <p><i class="fas fa-comments me-2 text-sky-400"></i><strong>Chat:</strong> <a
                                        href="https://www.facebook.com/gamekey" target="_blank"
                                        class="text-sky-400 hover:underline">Facebook Messenger</a></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    @foreach($order->orderItems as $item)
                        <div class="flex justify-between items-center text-xs bg-[#101822] p-3 rounded border border-[#2a475e]/20">
                            <span class="text-[#c7d5e0]">
                                ...
                                <span class="text-sky-400">({{ $item->version->version_name ?? 'N/A' }})</span>
                                x{{ $item->quantity }}
                            </span>
                            <span
                                class="text-white font-medium">{{ number_format($item->price_at_purchase * $item->quantity) }}đ</span>
                        </div>
                    @endforeach
            ...
            <div class="max-w-md mx-auto bg-[#171a21] border border-[#2a475e]/30 p-8 rounded-sm text-center shadow-2xl">
                ...
                <div class="bg-white p-2 rounded-sm inline-block mb-6">
                    <img src="https://qr.sepay.vn/img?acc=153058888&bank=VIB&amount={{ $order->total_amount }}&des=ORDER{{ $order->id }}"
                        alt="QR Thanh toán" class="w-56 h-56 mx-auto">
                </div>
