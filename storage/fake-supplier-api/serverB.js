...
Object.keys(GAMES_DB).forEach(id => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        keyPool[id].push({
            key: `SUPPLIERB-${rawUuid.substring(0, 5)}-${rawUuid.substring(5, 10)}-${rawUuid.substring(10, 15)}`,
            used: false
        });
    }
});
...
function apiAuth(req, res, next) {
    ...
    if (!apiKey || apiKey !== 'SUPPLIER_B_DEMO_2026') {
...
app.get('/api/health', (req, res) => {
    res.json({
        ...
        service: 'Fake Supplier API B (204 Games)',
        stats: {
...
app.listen(PORT, () => {
    console.log(`╔══════════════════════════════════════════════╗`);
    console.log(`║      FAKE SUPPLIER API B - 204 GAMES MODE    ║`);
    console.log(`╠══════════════════════════════════════════════╣`);
    console.log(`║  Port:         ${PORT}                          ║`);
    console.log(`║  API Key:      SUPPLIER_B_DEMO_2026          ║`);
    console.log(`║  30% simulated error rate                    ║`);
    console.log(`║  3 keys per game (Total: 612 keys)           ║`);
    console.log(`╚══════════════════════════════════════════════╝`);
});